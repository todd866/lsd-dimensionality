...
\subsection{The Dissociation of Entropy}

Our central finding is a dissociation between electrophysiological and hemodynamic complexity. At the millisecond timescale (MEG), 5-HT2A agonism collapses the dominant oscillatory modes that normally constrain cortical activity, resulting in a measurable increase in effective dimensionality. This confirms the ``entropic'' prediction of increased repertoire size.

However, this expansion is invisible to the participation ratio of the BOLD signal. The ``slow manifold''---the low-frequency structure maintained by metabolic constraints and anatomical connectivity---is remarkably resilient to the rapid fluctuations induced by psychedelics. While the \emph{content} of consciousness becomes hyper-associative (as seen in the MEG desynchronization), the \emph{container} (the metabolic state space) remains stable.
...
\subsection{The Spectral Centroid Bridge}

The shift in spectral centroid ($+16.8\%$, $d = 0.82$) provides the bridge between MEG and fMRI findings. While the \emph{number} of dimensions ($D_{\mathrm{eff}}$) did not change in fMRI, the \emph{shape} of the eigenspectrum did. Variance redistributes from global network patterns toward finer-grained local activity---a flattening of the eigenspectrum where the first few principal components explain slightly less variance.

This mirrors the MEG desynchronization but is heavily filtered by the low-pass nature of the hemodynamic response function. The spectral centroid shift suggests that even at slow timescales, some signature of the psychedelic state propagates through---not as increased dimensionality, but as altered structure within a stable manifold.
